...
Task("Apply-DotnetCore-Transforms").Does(() => {
  ...
  Transform(publishFolder, "transforms", destination, null, excludePattern);
});
...
Task("Apply-Xml-Transform").Does(() => {
  ...
  var publishDestinationXConnect = configuration.XConnectRoot;
  if (publishLocal) {
    publishDestination = configuration.PublishWebFolder;
  }
  foreach(var layer in layers)
  {
    Transform(layer, "code", publishDestination, publishDestinationXConnect, null);
  }
});
