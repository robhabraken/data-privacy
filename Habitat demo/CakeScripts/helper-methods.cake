...
public void Transform(string rootFolder, string filter, string publishDestination, string publishDestinationXConnect, string[] excludePatterns)
{
  ...
  foreach (var file in xdtFiles) {
    ...
    if(file.FullPath.ToLower().Contains("xConnect".ToLower())) {
    	sourceTransform = $"{publishDestinationXConnect}\\{fileToTransform}";
    }
